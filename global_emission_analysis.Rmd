---
title: "Analisa Emisi Global dengan Teknik Deskriptif dan Clustering"
output: html_notebook
---

Proyek analisa ini dikerjakan sebagai bentuk pembelajaran dan penarapan ilmu-ilmu analisa yang telah dipelajari.

### Pendahuluan

Selamat datang dalam RNotebook ini yang akan membahas proyek mengenai emisi global. Proyek ini bertujuan untuk menganalisis dan memahami pola emisi gas rumah kaca utama, seperti CO2, CH4, dan N2O, yang berkontribusi terhadap perubahan iklim global. Emisi gas rumah kaca merupakan isu yang sangat penting dalam lingkup global karena memiliki dampak yang signifikan terhadap keseimbangan ekosistem bumi dan kualitas hidup manusia.
...
#### Memuat paket-paket yang relevan dan mengimpor data

Pada proyek ini kami akan menggunakan beberapa paket yang relevan, seperti 'dplyr', 'ggplot2', dan 'cluster' yang mana berguna untuk analisa klaster.
...
Untuk dataset yang digunakan adalah global_emission yang kami dapat dari link

```{r impor data}
df <- read.csv("/Users/mac/r_notebook_project/global_emissions.csv")
head(df)
```
### Data Pre-processing
...
head(df)
```
Melakukan pembersihan data, menghilangkan nilai yang kosong, duplikat dan transformasi data, termasuk mengubah nama kolom supaya terlihat representatif.
...
emission_df <- df # Membuat 'emission_df' dataframe
```
...
Pada tahap ini kita akan menjawab beberapa pertanyaan berdasarkan tujuan analisa kita.

**Pertanyaan 1**
Bagaimana tren emisi global CH4, N2O, dan CO2 dari tahun ke tahun?
...
```
**Pertanyaan 2**
Negara mana yang memiliki emisi CO2 tertinggi? Bagaimana perbandingannya dengan GDP dan populasi negara tersebut?
...
**Pertanyaan 3**
Apakah ada hubungan antara pertumbuhan GDP suatu negara dengan emisi CO2?
...
**Pertanyaan 4**
Bagaimanakah hubungan yang ditunjukkan antara populasi dari negara dengan emisi CO2?
...
**Pertanyaan 5**
Apa kontribusi masing-masing sumber emisi CO2 (seperti batu bara, minyak, gas, pembakaran) terhadap total emisi global?
...
Telah didapatkan value-value yang merepresentasikan sebagai sumber dari emisi CO2, selanjutnya membuat dataframe dari sumber-sumber yang sudah terdefinisi kemudian menghitung persentase kontribusi masing-masing sumber.
...
**Pertanyaan 6**
Bagaimana perbandingan emisi CO2 per kapita antara negara-negara?
...
Negara-negara timur tengah mendominasi urutan ini, dengan Qatar berada diposisi pertama diikuti Kuwait dan Uni Emirat Arab.

**Pertanyaan 7**
Apakah ada perbedaan dalam pola emisi antara kelompok negara yang berbeda berdasarkan analisis klaster?
...
Didapatkan nilai-nilai koordinat yang mewakili klastering kelompok mana sajakah yang termasuk dalam nilai-nilai tersebut. Selanjutnya kami melakukan perbandingan koordinat pusat antara kluster dan melihat perbedaan relatif dalam karakteristik, serta mem-visualisasikan teman kedalam **diagram batang** untuk melihat perbedaan absolut antara koordinat pusat, dan **diagram garis** untuk melihat tren perubahan relatif antara klaster.
...
Memodifikasi hasil klastering untuk mengelompokkan nama-nama negara yang termasuk dalam masing-masing klaster.
...
Berdasarkan hasil analisa yang telah kami lakukan, dapat dikatakan bahwa jumlah populasi ataupun kenaikan GDP suatu negara tidak terlalu berkontribusi terhadap tingginya emisi, meskipun terdapat hubungan positif yang ditunjukkan. Namun hubungan tersebut lemah maka dapat dikatakan bahwa 2 variabel tersebut tidak terlalu berhubungan secara signifikan.

Dengan membandingkan emisi CO2 per kapita antara negara-negara, kita dapat melihat perbedaan tingkat emisi antara negara-negara yang memiliki populasi berbeda. Analisis ini dapat memberikan pemahaman tentang negara-negara mana yang memiliki tingkat emisi CO2 yang tinggi atau rendah per penduduknya. Hal ini dapat membantu dalam mengidentifikasi negara-negara yang perlu mengadopsi kebijakan lingkungan yang lebih ketat untuk mengurangi emisi CO2 dan menghadapi tantangan perubahan iklim.

Selain itu, analisis ini juga dapat memberikan gambaran tentang peran populasi dalam kontribusi terhadap emisi CO2. Jika terdapat perbedaan signifikan antara negara-negara dengan populasi yang serupa, hal ini dapat menunjukkan perbedaan dalam pola konsumsi, sektor industri, atau kebijakan lingkungan di masing-masing negara.

Dalam analisis ini juga menyimpulkan koordinat pusat yang direpresentasikan oleh angka-angka negatif dan positif. Dalam konteks ini:
* Koordinat pusat yang lebih rendah (nilai negatif) menunjukkan bahwa kluster tersebut memiliki nilai fitur yang lebih rendah dalam skala yang telah diubah. Ini mengindikasikan bahwa negara-negara dalam kluster tersebut cenderung memiliki emisi CO2, GDP, atau populasi yang lebih rendah dibandingkan dengan negara-negara dalam kluster lain.
* Koordinat pusat yang lebih tinggi (nilai positif) menunjukkan bahwa kluster tersebut memiliki nilai fitur yang lebih tinggi dalam skala yang telah diubah. Ini mengindikasikan bahwa negara-negara dalam kluster tersebut cenderung memiliki emisi CO2, GDP, atau populasi yang lebih tinggi dibandingkan dengan negara-negara dalam kluster lain.

Dengan membandingkan koordinat pusat antara kluster, kita dapat melihat perbedaan relatif dalam karakteristik fitur-fitur tersebut. Misalnya, jika terdapat kluster dengan koordinat pusat yang lebih tinggi dalam emisi CO2 namun koordinat pusat yang rendah dalam GDP, hal ini dapat menunjukkan bahwa negara-negara dalam kluster tersebut memiliki tingkat emisi CO2 yang tinggi, tetapi pertumbuhan ekonominya mungkin belum sebanding.
